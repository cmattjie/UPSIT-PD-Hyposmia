...
from train_functions import prepare_experiment, train_and_evaluate
...
if __name__ == '__main__':
    ...
    MODELS = {'RidgeClassifier': RidgeClassifier(random_state=seed),
    ...
    for experiment in config["experiments"]:
        ...
        for model_name, model in MODELS.items():
            ...
            exp_results = train_and_evaluate(model, X_train, y_train, preprocess=preprocess, show=True, train=True)
            ...
            results.append({
                ...
                "model": model_name,
                "accuracy": exp_results['accuracy'].mean(),
                "specificity": exp_results['specificity'].mean(),
                "sensitivity": np.sqrt(exp_results['sensitivity'].mean()),
                "precision": exp_results['precision'].mean(),
                "f1": exp_results['f1'].mean(),
    ...
    if args.age_sex == True:
        results_df.to_csv(f"results/results_{hyposmia_cutoff}_seed{seed}_age&sex_{args.impute}.csv", index=False)
    else:
        results_df.to_csv(f"results/results_{hyposmia_cutoff}_seed{seed}_{args.impute}.csv", index=False)